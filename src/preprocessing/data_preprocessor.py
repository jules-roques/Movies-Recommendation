...
class DataPreprocessor:
    ...
    def fusion(self, train_matrix: np.ndarray, test_matrix: np.ndarray) :
        ...
        if intersection != 0:
            raise RuntimeError(f"Train/Test fusion error: {intersection} overlapping ratings detected.")
    ...
    def denormalize(self, matrix_standardized: np.ndarray) -> np.ndarray:
        ...
        if self.means is None:
            raise ValueError("User means and stds are not computed. Run normalize_by_user() first.")
        
        return matrix_standardized * self.stds + self.means
    
    def filter_by_threshold(self, matrix: np.ndarray, min_ratings_user: int = 5, min_ratings_movies: int = 5):
        """
    ...
    def preprocess(self, matrix: np.ndarray, min_ratings_user=0, min_ratings_item=0) -> np.ndarray:
        """
        Initialize the data preprocessor.
        ...
        matrix, _, _ = self.filter_by_threshold(matrix, min_ratings_user, min_ratings_item)
    ...
    def split(self, ratings: np.ndarray, test_size: float = 0.2):
        ...
        if overlap != 0:
            raise RuntimeError(f"Train/Test split error: {overlap} overlapping ratings detected.")
